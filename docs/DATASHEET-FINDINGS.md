...
| U1-2 | **Set pin electrical types**: pins 1, 2, 40, 41 → `power_in`; pin 3 (EN) → `input`; pins 4 to 39 → `bidirectional` | **PDF p.11**, Table 3-1 Pin Definitions, "Type" column: `P` = power, `I` = input, `I/O/T` = input/output/tristate | All 41 pins are `unspecified` in `Parts.kicad_sym`. ERC electrical checking is off board-wide. |
